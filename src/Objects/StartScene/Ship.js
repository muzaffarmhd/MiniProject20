...
class Ship{
    ...
    initialize(path, position){
        ...
        this.gltfLoader.load(path,(gltf) => {
            ...
            this.character.quaternion.identity();
            this.character.rotateY(Math.PI);
            ...
            rotationQuaternion.setFromAxisAngle(new CANNON.Vec3(0, 1, 0), 0);
            this.body.quaternion.copy(rotationQuaternion);
    ...
    addAnimation(animation){
        ...
        animation.forEach(animation => {
            const action = this.mixer.clipAction(animation); // Create a clipAction
            this.states[animation.name] = action; // Store it with the animation name as key
        });
        this.currentState=this._setState('Idle');
    }
    _setState(state){
        this.states[state].reset();
        this.states[state].setEffectiveTimeScale(1);
        this.states[state].setEffectiveWeight(1);
    ...
    UpdateState(nextState){
        ...
        this.states[nextState].reset();
        this.states[nextState].setEffectiveTimeScale(1);
        this.states[nextState].setEffectiveWeight(1);
        this.states[nextState].crossFadeFrom(this.states[this.currentState], 0.2, true);
        this.states[nextState].play();
    ...
    moveCharacter(direction, camera, deltaTime, movement, handler, speed=false){
        ...
        switch (movement){
            case 'backward':
                ...
                break;
            case 'left':
                direction.applyAxisAngle(this.rotationAxis, Math.PI / 16); // Rotate 90 degrees to the left
                break;
            case 'right':
                direction.applyAxisAngle(this.rotationAxis, -Math.PI / 16); // Rotate 90 degrees to the right
                break;
            case 'forward':
            default:
                break;
